...
public sealed class ApiErrorResponseMapperTests
{
    ...
    [Theory]
    [InlineData(ErrorCodes.PaymentNotFound, 404)]
    [InlineData(ErrorCodes.PaymentProviderError, 502)]
    [InlineData(ErrorCodes.PaymentWebhookInvalid, 400)]
    public void ResolveStatusCode_maps_payment_codes(string code, int expectedStatus) =>
        Assert.Equal(expectedStatus, ApiErrorResponseMapper.ResolveStatusCode(code));
