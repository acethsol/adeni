...
type FunnelStep = { label: string; count: number; color: string };

type ServiceMixItem = { label: string; count: number; percent: number };
    ...
    <div className="space-y-4">
      ...
      {showAdvancedAnalytics ? (
        <div className="grid gap-4 lg:grid-cols-3">
          <BusinessPortalCard>
            <h3 className="text-sm font-bold uppercase tracking-widest text-accent">Booking funnel</h3>
            <p className="mt-1 text-xs text-muted">Pending → confirmed → cancelled/rejected.</p>
            {funnel.length === 0 || totalBookings === 0 ? (
              <p className="mt-8 text-center text-sm text-muted">Not enough data yet.</p>
            ) : (
                  ...
                  <div key={step.label}>
                    ...
                    <div className="h-2 overflow-hidden rounded-full bg-subtle">
                      <div
                        ...
                        style={{ width: `${Math.max(8, (step.count / maxFunnel) * 100)}%` }}
                      />
                    </div>
                  </div>
                ))}
          ...
          <BusinessPortalCard>
            <h3 className="text-sm font-bold uppercase tracking-widest text-accent">Repeat customers</h3>
            <p className="mt-1 text-xs text-muted">Share of customers with more than one booking.</p>
            <p className="mt-8 text-center text-4xl font-bold text-foreground">
              {repeatCustomerRate == null ? "—" : `${Math.round(repeatCustomerRate * 100)}%`}
            </p>
            <p className="mt-2 text-center text-xs text-muted-foreground">
              {repeatCustomerRate == null ? "Need at least 2 customers" : "Customers who booked again"}
            </p>
          </BusinessPortalCard>

          <BusinessPortalCard>
            <h3 className="text-sm font-bold uppercase tracking-widest text-accent">Service mix</h3>
            <p className="mt-1 text-xs text-muted">Bookings by service offering.</p>
            {serviceMix.length === 0 ? (
              <p className="mt-8 text-center text-sm text-muted">No service data yet.</p>
            ) : (
                ...
                {serviceMix.map((item) => (
                  <li key={item.label}>
                    ...
                    <div className="mt-1 h-1.5 overflow-hidden rounded-full bg-subtle">
                      <div className="h-full rounded-full bg-accent/70" style={{ width: `${item.percent}%` }} />
                    </div>
                  </li>
                ))}
              </ul>
